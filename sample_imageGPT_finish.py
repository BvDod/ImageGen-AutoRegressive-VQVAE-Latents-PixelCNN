...
def train_imageGPT(settings_imageGPT, settings_VQVAE):
    ...
    input_data_test = torch.load("datasets/celebA/discrete_test.pt").long()
    input_data_test = input_data_test.reshape((input_data_test.shape[0], 1024))

    with torch.no_grad():
        image_tokens = input_data_test[6:7, :512]
    ...
    print(outputs)
    grid = plot_grid_samples_tensor(outputs.cpu())
